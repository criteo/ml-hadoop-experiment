...
from ml_hadoop_experiment.tensorflow.keras_bootstrap_helper import (
    build_eval_only_model,
    evaluate_bootstrap,
)
...
def compile_model(model, weighted):
    if weighted:
        model.compile(optimizer="adam", loss="mse", weighted_metrics=["RootMeanSquaredError"])
    else:
...
@pytest.fixture
def df():
    df = pd.DataFrame({"feature1": [13, 2, 6, 33, 5], "feature2": [3, 1, 7, 3, 9]})
...
def test_evaluate_bootstrap(model_build_fn, df):
    def label_format_fn(df):
        return [
            np.stack(df["expected_sum"] + df["noise"]),
...
def test_evaluate_bootstrap_weight_metrics(model_build_fn, df):
    ...
    np.random.seed(0)
    boot1 = df.iloc[np.random.randint(n, size=n)]
    results_1 = keras_model.evaluate(
        input_format_fn(boot1), label_format_fn(boot1), sample_weight=weight_format_fn(boot1)
    )
    boot2 = df.iloc[np.random.randint(n, size=n)]
    results_2 = keras_model.evaluate(
        input_format_fn(boot2), label_format_fn(boot2), sample_weight=weight_format_fn(boot2)
    )
