...
@pytest.mark.parametrize("global_sampling,pos_sampling,neg_sampling,count_result,label_is_null", [
    (1.0, 1.0, 1.0, 6, False),
    (0.0, 1.0, 1.0, 0, False),
    (1.0, 0.0, 1.0, 2, False),
    (1.0, 1.0, 0.0, 4, False),
...
def test_sampling(
    ...
    with mock.patch(
        ...
        df_result =\
            sample_with_predicate(df, global_sampling, pos_sampling, neg_sampling,
                                  df["label"] > 0, ["feature1", "feature2"])
...
def test_sampling_weight_column(local_spark_session):
    ...
    with mock.patch(
        ...
        df_result =\
            sample_with_predicate(df, 0.9, 0.5, 0.9,
                                  df["label"] > 0, ["feature1", "feature2"])
