...
@pytest.mark.parametrize("column_type,feature_spec,expected_result", [
    ...
    (pyspark.sql.types.ArrayType(pyspark.sql.types.LongType(), False),
     tf.io.FixedLenFeature([1], dtype=tf.int64), True),
    (pyspark.sql.types.ArrayType(pyspark.sql.types.LongType(), False),
     tf.io.FixedLenFeature([2], dtype=tf.int64), True),
    (pyspark.sql.types.ArrayType(pyspark.sql.types.IntegerType(), False),
     tf.io.FixedLenFeature([2], dtype=tf.int64), True),
    (pyspark.sql.types.ArrayType(pyspark.sql.types.IntegerType(), False),
     tf.io.FixedLenFeature([2, 2], dtype=tf.int64), True),
    (pyspark.sql.types.ArrayType(pyspark.sql.types.ArrayType(pyspark.sql.types.LongType(), False),
                                 False), tf.io.FixedLenFeature([2], dtype=tf.int64), False),
    (pyspark.sql.types.ArrayType(pyspark.sql.types.ArrayType(pyspark.sql.types.LongType(), False),
                                 False), tf.io.FixedLenFeature([2, 2], dtype=tf.int64), True),

    # test conversion to VarLenFeature
    (pyspark.sql.types.IntegerType(), tf.io.VarLenFeature(tf.int64), True),
    (pyspark.sql.types.ArrayType(pyspark.sql.types.IntegerType(), False),
     tf.io.VarLenFeature(tf.int64), True)
...
@pytest.mark.parametrize("structfield,name,feature_spec,expected_result", [
    # different name
    (pyspark.sql.types.StructField("tata", pyspark.sql.types.LongType()),
     "toto", tf.io.FixedLenFeature([], dtype=tf.int64), False),

    # incompatible data type
    (pyspark.sql.types.StructField("toto", pyspark.sql.types.LongType()),
     "toto", tf.io.FixedLenFeature([], dtype=tf.int32), False),
    ...
    (pyspark.sql.types.StructField("toto", pyspark.sql.types.LongType(), nullable=False),
     "toto", tf.io.FixedLenFeature([], dtype=tf.int64), True),

    (pyspark.sql.types.StructField("toto", pyspark.sql.types.LongType(), nullable=True),
     "toto", tf.io.FixedLenFeature([], dtype=tf.int64, default_value=1), True)
])
def test_is_structfield_compatible_with_feature_spec(structfield, name, feature_spec,
                                                     expected_result):
