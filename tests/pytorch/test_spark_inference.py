...
from ml_hadoop_experiment.common.spark_inference import artifact_type, SerializableObj
from ml_hadoop_experiment.pytorch.fixtures.test_models import (
    load_tokenizer, load_reducer, load_translator
)
...
def test_with_inference_column_with_multiple_artifacts(local_spark_session, serialize_objs):
    ...
    model: Union[torch.nn.Module, SerializableObj] = SerializableObj(ss, load_translator) \
        if serialize_objs else load_translator()
    tokenizer: Union[torch.nn.Module, SerializableObj] = SerializableObj(ss, load_tokenizer) \
        if serialize_objs else load_tokenizer()
    df_result = with_inference_column(
        df=df, artifacts=[model, tokenizer], input_cols=["data"], inference_fn=run_prediction,
        output_type=StringType(), batch_size=1, output_col="predictions", num_threads=1
    )
...
def test_with_inference_column_and_preprocessing(local_spark_session, on_gpu):
    ...
    with mock.patch("ml_hadoop_experiment.pytorch.spark_inference.torch.cuda.is_available") \
        ...
        df_result = with_inference_column_and_preprocessing(
            df=df, artifacts=model, input_cols=["feature1", "feature2"],
            preprocessing=preprocessing_fn, inference_fn=inference_fn, output_type=DoubleType(),
            batch_size=2, output_col="predictions",
            num_threads=1
        )
        pdf = df_result.toPandas()
...
def test_with_inference_column_and_preprocessing_computed_once(local_spark_session):
    ...
    def inference_fn(
        ...
        counter["value"] += 1
        return tuple(zip(
            features[0].numpy() + counter["value"],
            features[1].numpy() + counter["value"])
        )

    ss = local_spark_session
    df = ss.createDataFrame([(2., 3.), (12., 13.)], ["feature1", "feature2"])
    df_result = with_inference_column_and_preprocessing(
        df=df, artifacts={"value": 0}, input_cols=["feature1", "feature2"],
        preprocessing=preprocessing_fn, inference_fn=inference_fn,
        output_type=ArrayType(DoubleType()),
        batch_size=2, output_col="predictions",
        num_threads=1
    )
    df_result = df_result \
        .withColumn("predictions1", df_result["predictions"].getItem(0)) \
        .withColumn("predictions2", df_result["predictions"].getItem(1)) \
        .drop("predictions")
    pdf = df_result.toPandas()
    _compare(pd.Series([3., 13.]), pdf["predictions1"])
    _compare(pd.Series([4., 14.]), pdf["predictions2"])
...
def test_with_inference_column_computed_once(local_spark_session):
    ...
    def inference_fn(
        ...
        counter["value"] += 1
        return pd.Series(zip(
            features[0] + counter["value"],
    ...
    df_result = with_inference_column(
        df=df, artifacts={"value": 0}, input_cols=["feature1", "feature2"],
        inference_fn=inference_fn, output_type=ArrayType(DoubleType()),
        batch_size=2, output_col="predictions",
        num_threads=1
    )
    df_result = df_result \
...
def run_with_inference_column(local_spark_session, serialize_model: bool, on_gpu: bool):
    def run_prediction(
        ...
        predictions = model(
            torch.Tensor(features[0].tolist()),
            torch.Tensor(features[1].tolist())
        )
    ...
    df = ss.createDataFrame(data, ["feature1", "feature2"])
    model: Union[torch.nn.Module, SerializableObj] = SerializableObj(ss, load_reducer) \
    ...
    with mock.patch("ml_hadoop_experiment.pytorch.spark_inference.torch.cuda.is_available") \
        ...
        df_result = with_inference_column(
            df=df, artifacts=model, input_cols=["feature1", "feature2"],
            inference_fn=run_prediction, output_type=DoubleType(), batch_size=1,
            output_col="predictions", num_threads=1
        )
        pdf = df_result.toPandas()
